...
window.loadAndDisplayUserProfile = async function(user) { 
    const s = await chatDb.ref(`users/${user.uid}`).once("value"); 
    const p = s.val() || {}; 
    usersCache[user.uid] = p; 
    ...
    setImage(document.getElementById("profilePreview"), p.photo); 
...
window.saveProfile = async function() {
    ...
    const status = statusInput?.value?.trim() || "Available";
...
window.shareProfile = function() {
    ...
    const profileData = {
        uid: user.uid,
        name: usersCache[user.uid]?.username || user.email,
        email: user.email
    };
    
    const shareText = `Check out my ChatMate profile: ${window.location.origin}/discover.html?user=${user.uid}`;
    
    if (navigator.share) {
        navigator.share({
            title: 'My ChatMate Profile',
            text: `Connect with me on ChatMate!`,
            url: `${window.location.origin}/discover.html?user=${user.uid}`
        }).catch(() => {
...
window.startCall = function(type) {
    if (!currentPeerId) {
        showToast("Select a friend to call first", "error");
        return;
    }
    
    // Check if peer is online
    const peer = usersCache[currentPeerId];
    if (!peer || !peer.online) {
        showToast(`${getDisplayName(peer)} is offline`, "error");
        return;
    }
    
    showToast(`${type} calling will be available in the next update!`, "info");
    // Future implementation: WebRTC integration
};
...
async function renderUsers() {
    ...
    if (friendIds.length === 0) { 
        userList.innerHTML = `<div class="empty-state"><p>😊 No friends yet</p><a href="discover.html" class="discover-link">🌍 Discover People</a></div>`; 
        return; 
    }
    ...
    for (const fid of friendIds) {
        ...
        if (!profile.username) { 
            const ps = await chatDb.ref(`users/${fid}`).once("value"); 
            profile = ps.val() || {}; 
            usersCache[fid] = profile; 
        }
        toDisplay.push({ 
            uid: fid, 
            profile, 
            lastMessageTime: profile.lastMessageTime || 0, 
            unreadCount: unreadMessages[fid] || 0, 
    ...
    btn.className = "discover-friends-btn"; 
    btn.innerHTML = `<button onclick="window.location.href='discover.html'" class="discover-btn">🌍 Discover New Friends</button>`; 
...
function updateChatHeader(peer) { 
    ...
    const badge = document.getElementById("friendStatusBadge"); 
    
    if (!peer) { 
        if (name) name.textContent = "Choose someone to chat"; 
        if (status) status.textContent = "Select a person from Messages."; 
        setImage(photo, ""); 
        if (badge) badge.style.display = "none"; 
        return; 
    } 
    
    if (name) name.textContent = getDisplayName(peer); 
    if (status) status.textContent = peer.online ? "🟢 Online" : `📅 Last seen ${formatMessageTime(peer.lastSeen)}`; 
...
function renderMessage(s) { 
    const d = s.val(); 
    if (!d) return; 
    const isMine = auth.currentUser && d.senderId === auth.currentUser.uid; 
    const msg = document.createElement("div"); 
    msg.className = isMine ? "message mine" : "message theirs"; 
    msg.innerHTML = `<div><strong>${escapeHtml(getDisplayName({ username: d.senderName, email: d.senderEmail }))}</strong></div>${d.image ? `<img class="message-photo" src="${d.image}" onclick="window.open('${d.image}','_blank')">` : `<div>${escapeHtml(d.text || "")}</div>`}<span>${formatMessageTime(d.time)}</span>${d.seen && isMine ? '<span>✓ Seen</span>' : ''}`; 
    if (chatBox) {
...
function listenToChat(chatId, peerId) { 
    ...
    currentMessagesRef.orderByChild("time").on("child_added", m => { 
        renderMessage(m); 
        if (m.val().senderId !== auth.currentUser?.uid) 
            chatDb.ref(`messages/${chatId}/${m.key}/seen`).set(true); 
    }); 
    ...
    currentTypingRef.on("value", s => {
        if (ind) ind.textContent = s.exists() ? "✏️ Typing..." : "";
    }); 
...
async function openChat(userId) { 
    ...
    const f = await chatDb.ref(`users/${user.uid}/friends/${userId}`).once("value"); 
    if (!f.exists()) { 
        alert("❌ You can only chat with accepted friends."); 
        return; 
    } 
...
window.sendMessage = async function() {
    ...
    if (!areFriends && currentPeerId !== user.uid) { 
    ...
    if (usersCache[currentPeerId]) { 
        ...
        usersCache[currentPeerId].lastMessageTime = ts; 
        renderUsers(); 
    }
...
window.typing = () => { 
    if (auth.currentUser && currentChat) {
        chatDb.ref("typing/" + currentChat + "/" + auth.currentUser.uid).set(true);
        if (typingTimeout) clearTimeout(typingTimeout);
        typingTimeout = setTimeout(() => {
            chatDb.ref("typing/" + currentChat + "/" + auth.currentUser.uid).remove();
        }, 2000);
    }
};
...
// Safe media file listener
const mediaFileElement = document.getElementById("mediaFile");
if (mediaFileElement) {
    mediaFileElement.addEventListener("change", async (e) => { 
        if (e.target.files[0] && currentChat && currentPeerId) { 
            ...
            reader.onload = async (ev) => { 
                ...
                await chatDb.ref("messages/" + currentChat).push({ 
                    ...
                    senderName: getDisplayName(usersCache[auth.currentUser.uid] || {}), 
                    receiverId: currentPeerId, 
                    seen: false, 
                    time: Date.now(), 
                    image: compressed 
...
window.logout = async () => { 
    if (auth.currentUser) {
        await chatDb.ref("users/" + auth.currentUser.uid).update({ online: false, lastSeen: Date.now() });
    }
    // Clean up listeners
    if (currentMessagesRef) currentMessagesRef.off();
    if (currentTypingRef) currentTypingRef.off();
...
auth.onAuthStateChanged(async user => {
    ...
    messagesListenerRef = chatDb.ref("messages").on("child_added", s => { 
        const msgs = s.val(); 
        if (msgs) {
            Object.keys(msgs).forEach(k => { 
                const m = msgs[k]; 
                if (m && m.receiverId === user.uid && !m.seen && m.senderId !== currentPeerId) { 
                    unreadMessages[m.senderId] = (unreadMessages[m.senderId] || 0) + 1; 
                    renderUsers(); 
                } 
            });
        }
    ...
    // Periodic user refresh (reduced frequency to save costs)
    userStatusInterval = setInterval(async () => { 
        const s = await chatDb.ref("users").once("value"); 
        usersCache = s.val() || {}; 
        renderUsers(); 
        if (currentPeerId && usersCache[currentPeerId]) updateChatHeader(usersCache[currentPeerId]); 
    }, 60000);
    ...
    if (stored) { 
        try { 
            ...
            setTimeout(async () => { 
                if (usersCache[userId]) openChat(userId); 
                else { 
                    const us = await chatDb.ref(`users/${userId}`).once("value"); 
                    if (us.val()) { 
                        usersCache[userId] = us.val(); 
                        openChat(userId); 
                    } 
                } 
            }, 1000); 
...
chatDb.ref("users").on("value", async s => { 
    usersCache = s.val() || {}; 
    await renderUsers(); 
    if (currentPeerId && usersCache[currentPeerId]) updateChatHeader(usersCache[currentPeerId]); 
});
...
>>>>>>> d07fbf3045d40aca2beac096e05515a6db69bdff
